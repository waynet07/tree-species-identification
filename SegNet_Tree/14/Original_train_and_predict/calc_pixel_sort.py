...
def ConfusionMatrixPlot(confmatrix):    
    ...
    plt.title('Confusion matrix of SegNet for Original images',fontsize=15,pad=10)
    iters = np.reshape([[[i, j] for j in range(len(clsnames))] for i in range(len(clsnames))], (confmatrix.size, 2))
    for i, j in iters:
        plt.text(j, i, format(confmatrix[i, j]), fontsize=7, va='center', ha='center')  # 显示对应的数字
    ...
    plt.xlabel('Predict Species',labelpad=10,fontsize=15)
