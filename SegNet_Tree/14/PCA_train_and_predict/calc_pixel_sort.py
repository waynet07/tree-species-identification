...
def ConfusionMatrixPlot(confmatrix):    
    ...
    plt.imshow(confmatrix, interpolation='nearest', cmap='cool')  # 按照像素显示出矩阵 cmap=plt.cm.BuPu
    plt.xticks(tick_marks, clsnames+1)
    plt.yticks(tick_marks, clsnames+1)

    plt.ylabel('Actual classes',labelpad=-5,fontsize=15)
    plt.xlabel('Predicted classes',labelpad=10,fontsize=15)
    plt.ylim([len(clsnames) - 0.5, -0.5])
    plt.tight_layout()

    cb=plt. colorbar()# heapmap
    #cb.set_label(label='aa',fontsize = 15)
    plt.savefig('./Results/SegNet_PCA_14_Matrix.png')
