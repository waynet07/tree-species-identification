...
### Generate Results
classification_report_out = "./Results/14_VGG19_Feature.txt"
f = open(classification_report_out, 'w')

#数据的目录
data_dir_train='./14/Feature/train'
#构造pathlib路径对象
...
def plot_learning_curves(history):
    ...
    plt.title('Training / Validation Loss and Accuracy on VGG19 (CS-based Photographs)',y=1.05)
    plt.grid(True)
    ...
    plt.savefig('./Results/VGG19_Feature_14_curves.png')
   # plt.show()
...
pd.DataFrame(confmatrix).to_csv('./Results/14_VGG19_Feature_confusion_matrix.csv')
...
def ConfusionMatrixPlot(confmatrix):    
   ...
   #cb.set_label('Numbers of Predict',fontsize = 15)
    plt.savefig('./Results/VGG19_Feature_14_Matrix.png')
