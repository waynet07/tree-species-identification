...
### Generate Results
classification_report_out = "./Results/14_VGG19_Original.txt"
f = open(classification_report_out, 'w')

#数据的目录
data_dir_train='./14/Original/train'
#构造pathlib路径对象
...
#test通道设置
data_dir_test='./14/Original/test'
...
def plot_learning_curves(history):
    ...
    plt.savefig('./Results/VGG19_Original_14_curves.png')
    #plt.show()
...
pd.DataFrame(confmatrix).to_csv('./Results/14_VGG19_Original_confusion_matrix.csv')
...
def ConfusionMatrixPlot(confmatrix):    
    ...
    plt.title('Confusion matrix of VGG19 for original photographs',fontsize=15,pad=10)
   ...
   #cb.set_label('Numbers of Predict',fontsize = 15)
    plt.savefig('./Results/VGG19_Original_14_Matrix.png')
