...
### Generate Results
classification_report_out = "./Results/14_VGG19_PCA.txt"
f = open(classification_report_out, 'w')

#数据的目录
data_dir_train='./14/PCA/train'
#构造pathlib路径对象
...
for n in range(3):
    image_index_train=random.choice(range(len(all_image_path_train)))
    display.display(display.Image(all_image_path_train[image_index_train]))
    print (index_to_label_train[all_image_label_train[image_index_train]])
    print()
...
for n in range(3):
    image_index_val=random.choice(range(len(all_image_path_val)))
    display.display(display.Image(all_image_path_val[image_index_val]))
    print (index_to_label_val[all_image_label_val[image_index_val]])
    print()
...
def plot_learning_curves(history):
    ...
    plt.title('Training / Validation Loss and Accuracy on VGG19 (Original Photographs)',y=1.05)
    plt.grid(True)
    ...
    plt.savefig('./Results/VGG19_PCA_14_curves.png')
    #plt.show()
...
print("VGG19 Training Time: {}:{:.2f}".format(mins, secs),(model.evaluate(test_dataset,verbose=0)))
...
pd.DataFrame(confmatrix).to_csv('./Results/14_VGG19_PCA_confusion_matrix.csv')
...
def ConfusionMatrixPlot(confmatrix):    
    ...
    plt.title('Confusion matrix of VGG19 for PCA-based photographs',fontsize=15,pad=10)
    iters = np.reshape([[[i, j] for j in range(len(clsnames))] for i in range(len(clsnames))], (confmatrix.size, 2))
    for i, j in iters:
        plt.text(j, i, format(confmatrix[i, j]), fontsize=12, va='center', ha='center')  # 熱力圖 對應文字
    ...
    plt.imshow(confmatrix, interpolation='nearest', cmap='cool')  # 按照像素顯示矩陣
   ...
   #cb.set_label('Numbers of Predict',fontsize = 15)
    plt.savefig('./Results/VGG19_PCA_14_Matrix.png')
